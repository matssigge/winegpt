// Generated by ReScript, PLEASE EDIT WITH CARE

import * as $$String from "rescript/lib/es6/string.js";
import * as Belt_Int from "rescript/lib/es6/belt_Int.js";
import * as EntryApi from "./EntryApi.bs.js";
import * as ApiClient from "../api/ApiClient.bs.js";
import * as Belt_Array from "rescript/lib/es6/belt_Array.js";
import * as Caml_array from "rescript/lib/es6/caml_array.js";
import * as Belt_Option from "rescript/lib/es6/belt_Option.js";
import * as Js_promise2 from "rescript/lib/es6/js_promise2.js";
import * as ResponseDecoder from "../api/ResponseDecoder.bs.js";

function initialStatus() {
  return "Idle";
}

function loadingStatus() {
  return "Loading";
}

function emptyStatus() {
  return {
          TAG: "Ready",
          _0: []
        };
}

function readyStatus(entries) {
  return {
          TAG: "Ready",
          _0: entries
        };
}

function errorStatus(message) {
  return {
          TAG: "Error",
          _0: message
        };
}

function updateForm(form, field, value) {
  switch (field) {
    case "consumedAt" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: value,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "grape" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: value,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "locationText" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: value,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "pairingNotes" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: value,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "producer" :
        return {
                wineName: form.wineName,
                producer: value,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "rating" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: value,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "tastingNotes" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: value,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "venueName" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: value,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "vintage" :
        return {
                wineName: form.wineName,
                producer: form.producer,
                grape: form.grape,
                vintage: value,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    case "wineName" :
        return {
                wineName: value,
                producer: form.producer,
                grape: form.grape,
                vintage: form.vintage,
                consumedAt: form.consumedAt,
                venueName: form.venueName,
                locationText: form.locationText,
                pairingNotes: form.pairingNotes,
                tastingNotes: form.tastingNotes,
                rating: form.rating,
                isSubmitting: form.isSubmitting,
                error: undefined,
                success: undefined
              };
    default:
      return form;
  }
}

function startSubmitting(form) {
  return {
          wineName: form.wineName,
          producer: form.producer,
          grape: form.grape,
          vintage: form.vintage,
          consumedAt: form.consumedAt,
          venueName: form.venueName,
          locationText: form.locationText,
          pairingNotes: form.pairingNotes,
          tastingNotes: form.tastingNotes,
          rating: form.rating,
          isSubmitting: true,
          error: undefined,
          success: undefined
        };
}

function failForm(form, message) {
  return {
          wineName: form.wineName,
          producer: form.producer,
          grape: form.grape,
          vintage: form.vintage,
          consumedAt: form.consumedAt,
          venueName: form.venueName,
          locationText: form.locationText,
          pairingNotes: form.pairingNotes,
          tastingNotes: form.tastingNotes,
          rating: form.rating,
          isSubmitting: false,
          error: message,
          success: undefined
        };
}

function succeedForm() {
  return {
          wineName: "",
          producer: "",
          grape: "",
          vintage: "",
          consumedAt: "",
          venueName: "",
          locationText: "",
          pairingNotes: "",
          tastingNotes: "",
          rating: "",
          isSubmitting: false,
          error: undefined,
          success: "Entry saved."
        };
}

function padTwo(value) {
  var text = String(value);
  if (value < 10) {
    return "0" + text;
  } else {
    return text;
  }
}

function toDateTimeLocalValue(consumedAt) {
  var date = new Date(consumedAt);
  if (Number.isNaN(date.getTime())) {
    return "";
  } else {
    return String(date.getFullYear()) + "-" + padTwo(date.getMonth() + 1 | 0) + "-" + padTwo(date.getDate()) + "T" + padTwo(date.getHours()) + ":" + padTwo(date.getMinutes());
  }
}

function formFromEntry(entry) {
  return {
          wineName: entry.wine.name,
          producer: Belt_Option.getWithDefault(entry.wine.producer, ""),
          grape: Belt_Option.getWithDefault(entry.wine.grape, ""),
          vintage: Belt_Option.getWithDefault(Belt_Option.map(entry.wine.vintage, (function (prim) {
                      return String(prim);
                    })), ""),
          consumedAt: toDateTimeLocalValue(entry.consumedAt),
          venueName: Belt_Option.getWithDefault(entry.venueName, ""),
          locationText: Belt_Option.getWithDefault(entry.locationText, ""),
          pairingNotes: Belt_Option.getWithDefault(entry.pairingNotes, ""),
          tastingNotes: Belt_Option.getWithDefault(entry.tastingNotes, ""),
          rating: Belt_Option.getWithDefault(Belt_Option.map(entry.rating, (function (prim) {
                      return String(prim);
                    })), ""),
          isSubmitting: false,
          error: undefined,
          success: undefined
        };
}

function entries(status) {
  if (typeof status !== "object") {
    return [];
  } else if (status.TAG === "Ready") {
    return status._0;
  } else {
    return [];
  }
}

function selectedEntry(entries, selectedEntryId) {
  if (selectedEntryId !== undefined) {
    return Belt_Array.getBy(entries, (function (entry) {
                  return entry.id === selectedEntryId;
                }));
  }
  
}

function resolveSelectedEntryId(entries, selectedEntryId) {
  if (entries.length === 0) {
    return ;
  }
  var match = selectedEntry(entries, selectedEntryId);
  if (match !== undefined) {
    return selectedEntryId;
  } else {
    return Caml_array.get(entries, 0).id;
  }
}

function appendEntry(entries, entry) {
  return Belt_Array.concat([entry], entries);
}

function replaceEntry(entries, nextEntry) {
  return Belt_Array.map(entries, (function (entry) {
                if (entry.id === nextEntry.id) {
                  return nextEntry;
                } else {
                  return entry;
                }
              }));
}

function isReady(status) {
  if (typeof status !== "object" || status.TAG !== "Ready") {
    return false;
  } else {
    return true;
  }
}

function stringOption(value) {
  var trimmed = $$String.trim(value);
  if (trimmed === "") {
    return ;
  } else {
    return trimmed;
  }
}

function intOption(value, errorCode) {
  var trimmed = $$String.trim(value);
  if (trimmed === "") {
    return Promise.resolve(undefined);
  }
  var number = Belt_Int.fromString(trimmed);
  if (number !== undefined) {
    return Promise.resolve(number);
  } else {
    return Promise.reject(new Error(errorCode));
  }
}

function normalizeConsumedAt(value) {
  var trimmed = $$String.trim(value);
  if (trimmed === "") {
    return Promise.reject(new Error("invalid_consumed_at"));
  }
  var date = new Date(trimmed);
  if (Number.isNaN(date.getTime())) {
    return Promise.reject(new Error("invalid_consumed_at"));
  } else {
    return Promise.resolve(date.toISOString());
  }
}

function listEntries(token, collectionId) {
  return Js_promise2.then(EntryApi.listEntries(token, collectionId), (function (response) {
                var entries = Belt_Option.flatMap(ResponseDecoder.parse(response), ResponseDecoder.entries);
                if (entries !== undefined) {
                  return Promise.resolve(entries);
                } else {
                  return Promise.reject(ApiClient.invalidResponse());
                }
              }));
}

function createEntry(token, collectionId, form) {
  return Js_promise2.then(normalizeConsumedAt(form.consumedAt), (function (consumedAt) {
                return Js_promise2.then(intOption(form.vintage, "invalid_wine_vintage"), (function (vintage) {
                              return Js_promise2.then(intOption(form.rating, "invalid_rating"), (function (rating) {
                                            var producer = stringOption(form.producer);
                                            var grape = stringOption(form.grape);
                                            var venueName = stringOption(form.venueName);
                                            var locationText = stringOption(form.locationText);
                                            var pairingNotes = stringOption(form.pairingNotes);
                                            var tastingNotes = stringOption(form.tastingNotes);
                                            return Js_promise2.then(EntryApi.createEntry(token, collectionId, producer, form.wineName, vintage, undefined, grape, undefined, undefined, consumedAt, venueName, locationText, pairingNotes, tastingNotes, rating, undefined), (function (response) {
                                                          var entry = Belt_Option.flatMap(ResponseDecoder.parse(response), ResponseDecoder.entry);
                                                          if (entry !== undefined) {
                                                            return Promise.resolve(entry);
                                                          } else {
                                                            return Promise.reject(ApiClient.invalidResponse());
                                                          }
                                                        }));
                                          }));
                            }));
              }));
}

function updateEntry(token, collectionId, entryId, form) {
  return Js_promise2.then(normalizeConsumedAt(form.consumedAt), (function (consumedAt) {
                return Js_promise2.then(intOption(form.vintage, "invalid_wine_vintage"), (function (vintage) {
                              return Js_promise2.then(intOption(form.rating, "invalid_rating"), (function (rating) {
                                            var producer = stringOption(form.producer);
                                            var grape = stringOption(form.grape);
                                            var venueName = stringOption(form.venueName);
                                            var locationText = stringOption(form.locationText);
                                            var pairingNotes = stringOption(form.pairingNotes);
                                            var tastingNotes = stringOption(form.tastingNotes);
                                            return Js_promise2.then(EntryApi.updateEntry(token, collectionId, entryId, producer, form.wineName, vintage, undefined, grape, undefined, undefined, consumedAt, venueName, locationText, pairingNotes, tastingNotes, rating, undefined), (function (response) {
                                                          var entry = Belt_Option.flatMap(ResponseDecoder.parse(response), ResponseDecoder.entry);
                                                          if (entry !== undefined) {
                                                            return Promise.resolve(entry);
                                                          } else {
                                                            return Promise.reject(ApiClient.invalidResponse());
                                                          }
                                                        }));
                                          }));
                            }));
              }));
}

var initialForm = {
  wineName: "",
  producer: "",
  grape: "",
  vintage: "",
  consumedAt: "",
  venueName: "",
  locationText: "",
  pairingNotes: "",
  tastingNotes: "",
  rating: "",
  isSubmitting: false,
  error: undefined,
  success: undefined
};

export {
  initialStatus ,
  loadingStatus ,
  emptyStatus ,
  readyStatus ,
  errorStatus ,
  initialForm ,
  updateForm ,
  startSubmitting ,
  failForm ,
  succeedForm ,
  padTwo ,
  toDateTimeLocalValue ,
  formFromEntry ,
  entries ,
  selectedEntry ,
  resolveSelectedEntryId ,
  appendEntry ,
  replaceEntry ,
  isReady ,
  stringOption ,
  intOption ,
  normalizeConsumedAt ,
  listEntries ,
  createEntry ,
  updateEntry ,
}
/* EntryApi Not a pure module */
